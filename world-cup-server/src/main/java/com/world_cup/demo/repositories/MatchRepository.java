...
public interface MatchRepository extends JpaRepository<Match,Integer> {
    ...
    @Query(value = "select match.odds_api_id,match.id,match.competition, match.match_id, match.away_team, match.home_team, " +
            ...
            "from match where (match.match_date::timestamptz AT TIME ZONE 'America/New_York')::date = CAST(:date AS date)", nativeQuery = true)
    ...
    @Query(value = "select * from match where ((home_team = ?1 and away_team = ?2) or (home_team = ?2 and away_team = ?1)) and (match_date::timestamptz AT TIME ZONE 'America/New_York')::date = cast(?3 as date) order by match_date asc limit 1", nativeQuery = true)
